...
double K_Best_matching_rotation_upper(vector<MatrixXd> As,vector<MatrixXd> Bs,Answer pre,int K,bool mirror,Cube C_r,Cube C_t,vector<vector<int> > a_assign,vector<vector<int> > b_assign){
    ...
    for(int s=1;s<K;s++){
        ...
        // for(int i=0;i<assignments[0].size();i++){
        //     cout << assignments[0][i];
        // }
        // cout << endl;
        if(ans_candidate < pre.E){
            pre.E = ans_candidate;
...
Answer K_Best_matching_rotation(vector<MatrixXd> As, vector<MatrixXd> Bs,double pro_solution,double pro_lower,int K,double &lower,bool &Flag,bool mirror,Cube C_r,Cube C_t,vector<vector<int> > a_assign,vector<vector<int> > b_assign){
    ...
    for(int s=0;s<K;s++){
        ...
        if(Cost.cost>=pro_lower){
            // cout << endl;
            // for(int i=0;i<Lower_candidates[0].size();i++){
            //     cout << Lower_candidates[0][i].lower << endl; 
            // }
            // cout << endl;
            // cout << K << endl;
            return Ans;
...
//         vector<vector<double> > M = caluculate_lower_graph(A[iter],B[iter],R,C.diameter);
//         vector<int> assignment;
//         cost += HungAlgo.Solve(M, assignment);
//     }
//     return cost;
...
// double calculate_upper_rotation(Cube C_r,vector<MatrixXd> As, vector<MatrixXd> Bs,Cube C_t){
//     HungarianAlgorithm HungAlgo;
//     MatrixXd R = make_rotation(C_r.center,C_r.mirror);
//     double cost = 0;
//     for(int iter=0;iter<As.size();iter++){
//         int a = As[iter].cols();
//         int b = Bs[iter].cols();
...
// double calculate_lower_rotation(Cube C_r,vector<MatrixXd> As, vector<MatrixXd> Bs,Cube C_t){
//     HungarianAlgorithm HungAlgo;
//     MatrixXd R = make_rotation(C_r.center,C_r.mirror);
//     double cost = 0;
//     for(int iter=0;iter<As.size();iter++){
...
//                 }
//                 M[i][j] = sqrt(M[i][j]);
//                 M[i][j] = M[i][j]- gamma[i]-C_t.diameter;
//                 M[i][j] = max(M[i][j],(double)0)*max(M[i][j],(double)0);
//             }
//         }
//         vector<int> assignment;
...
// 				continue;
// 			}
//             double E_upper = calculate_upper_rotation(CC,As,Bs,C_t);
// 			CC.upper = E_upper;
// 			if(E_upper<ans.E){
// 				ans.E = E_upper;
//                 ans.t = C_t.center;
//                 ans.mirror = CC.mirror;
//                 ans.R = make_rotation(CC.center,CC.mirror);
// 			}
// 			double E_lower = calculate_lower_rotation(CC,As,Bs,C_t);
// 			CC.lower = E_lower;
// 			if(E_lower>ans.E){
// 				continue;
// 			}
...
//     MatrixXd R = make_rotation(C.center,mirror);
//     for(int iter=0;iter<A.size();iter++){
//         vector<vector<double> > M = caluculate_lower_graph_fix(A[iter],B[iter],R,C.diameter);
//         Ms.push_back(M);
//     }
//     vector<priority_queue<solution_space> > ques;
//     vector<vector<Lower_candidate> > Lower_candidates(A.size());
//     for(int iter=0;iter<A.size();iter++){
...
//             for(int i=0;i<b;i++){
//                 if(Lower_candidates[iter][froms[iter]].assignments[i] != -1){
//                     T(i,Lower_candidates[iter][froms[iter]].assignments[i]) = 1;
//                 }
//             }
//             Ts.push_back(T);
//         }

//         MatrixXd R = calculate_from_assignment(Ts,A,B,mirror);
//         double ans_candidate = calculate_ans(A,B,R,assignments);
//         if(ans_candidate < pre.E){
//             pre.E = ans_candidate;
//             pre.R = R;
//         }
...
bool calculate_lower_K_best_eps(Cube C,vector<MatrixXd> As,vector<MatrixXd> Bs,bool mirror,vector<Answer> &ans_multiple,double eps,int k,vector<vector<int> > a_assign,vector<vector<int> > b_assign){
    HungarianAlgorithm HungAlgo;
    vector<vector<vector<double> > > Ms;
    int N = 0;
    int len_a = 0;
...
// // bool calculate_lower_K_best(Cube C,vector<MatrixXd> A,vector<MatrixXd> B,bool mirror,Answer &pre,double &lower,int k){
// //     HungarianAlgorithm HungAlgo;
// //     vector<vector<vector<double> > > Ms;
// //     MatrixXd R = make_rotation(C.center,mirror);
// //     for(int iter=0;iter<A.size();iter++){
...
// //         solution_space Sol;
// //         edge E;
// //         Sol.second_largest = cost + difference_second_largest_Dijkstra_edge_change_assignment(assignment2,MM,E);
// //         Sol.assignment = assignment2;
// //         Sol.E = E;
// //         Sol.I = {};
// //         Sol.O = {};
...
// //                 double cost = HungAlgo.Solve(M1, assignment_2);
// //                 edge E;
// //                 Sol1.second_largest = sqrt(cost + difference_second_largest_Dijkstra_edge_change_assignment(assignment_2,M1,E)); 
// //                 Sol1.E = E;
// //                 Sol1.assignment = assignment_2;
// //                 ques[iter].push(Sol1);
// //                 solution_space Sol2;
...
// //                 vector<int> assignment_3(a);
// //                 cost = HungAlgo.Solve(M2, assignment_3);
// //                 Sol2.second_largest = cost + difference_second_largest_Dijkstra_edge_change_assignment(assignment_3,M2,E);
// //                 Sol2.E = E;
// //                 Sol2.assignment = assignment_3;
// //                 ques[iter].push(Sol2);
// //                 solution_space Sol_f = ques[iter].top();
...
// bool check_in_sphere_64(Cube_Cube C){
//     double r = 0;
//     for(int i=0;i<3;i++){
//         r += C.center_r[i]*C.center_r[i];
//     }
//     if(sqrt(r) < C.diameter_r + PI){
//         return true;
//     }else{
//         return false;
//     }
...
//     HungarianAlgorithm HungAlgo;
//     vector<vector<vector<double> > > Ms;
//     MatrixXd R = make_rotation(C.center_r,mirror);
//     for(int iter=0;iter<A.size();iter++){
//         vector<vector<double> > M = caluculate_lower_graph_64(A[iter],B[iter],R,C.diameter_r,C.center_t,C.diameter_t);
//         Ms.push_back(M);
//     }
//     vector<priority_queue<solution_space> > ques;
//     vector<vector<Lower_candidate> > Lower_candidates(A.size());
