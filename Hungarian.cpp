...
// double HungarianAlgorithm::Solve_modify(vector <vector<double> >& DistMatrix, vector<int>& Assignment,vector <vector<double> >& Modify_DistMatrix)
// {
...
// 	assignmentoptimal(assignment, &cost, distMatrixIn, nRows, nCols,Modyfy_DistMatrix);

// 	Assignment.clear();
// 	for (unsigned int r = 0; r < nRows; r++)
// 		Assignment.push_back(assignment[r]);

// 	Modify_DistMatrix.clear();
// 	for (unsigned int r = 0; r < nRows; r++)
// 		Assignment.push_back(assignment[r]);
...
double HungarianAlgorithm::Solve(vector <vector<double> >& DistMatrix, vector<int>& Assignment)
{
    ...
    double *distMatrix;
	distMatrix = (double *)malloc((nRows*nCols) * sizeof(double));
	...
	// call solving function
	assignmentoptimal(assignment, &cost, distMatrixIn, nRows, nCols,distMatrix);

	for (unsigned int i = 0; i < nRows; i++)
		for (unsigned int j = 0; j < nCols; j++)
			DistMatrix[i][j] = distMatrix[i + nRows * j];
...
double HungarianAlgorithm::Solve2(vector <vector<double> >& DistMatrix, vector<int>& Assignment)
{
	unsigned int nRows = DistMatrix.size();
	unsigned int nCols = DistMatrix[0].size();
...
//********************************************************//
void HungarianAlgorithm::assignmentoptimal(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns,double *distMatrix)
{
