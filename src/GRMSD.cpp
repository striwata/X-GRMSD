...
int main(int argc, char *argv[]){
	...
	bool multiple_query = atoi(argv[6]);
	...
	if(multiple_query){
		...
		while(getline(ifs_csv_file, line)) {    
            ...
            while(getline(ifs_csv_file_data, line)) {
				...
				if(A.size() == B.size()){
					if(function_name == "AO"){
						Answer ans = AO_same_size(B,A,pen_model,pen_data,Smentai,permit_mirror);
						output(ans,outputfile);
					}
					if(function_name == "TSR"){
						Answer ans = TSR_same_size(B,A,pen_model,pen_data,Smentai,permit_mirror,0.3);
						output(ans,outputfile);
					}
					if(function_name == "IsometryOpt"){
						Answer ans = IsometryOpt_same_size(B,A,pen_model,pen_data,permit_mirror,1,1);
						output(ans,outputfile);
					}
					if(function_name == "MatchFastOpt"){
						Answer ans = MatchFastOpt_same_size(B,A,pen_model,pen_data,permit_mirror);
						output(ans,outputfile);
					}
				}else{
					if(function_name == "AO"){
						Answer ans = AO_diff_size(B,A,pen_model,pen_data,Smentai,permit_mirror);
						output(ans,outputfile);
					}
					if(function_name == "TSR"){
						cout << "TSR is not defined with molecules of different sizes" << endl;
					...
					if(function_name == "IsometryOpt"){
						Answer ans = IsometryOpt_diff_size(B,A,pen_model,pen_data,permit_mirror,1,1);
						output(ans,outputfile);
					}
					if(function_name == "MatchFastOpt"){
						Answer ans = MatchFastOpt_diff_size(B,A,pen_model,pen_data,permit_mirror);
						output(ans,outputfile);
					}
				}
			}
