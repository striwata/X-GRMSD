...
int main(int argc, char *argv[]){
    ...
    if(function_name == "Improved-MatchFPT"){
        vector<Answer> ans_multiple = Improved_MatchFPT(BB[0],AA[0],pen_model[0],pen_data[0],r,permit_mirror);
        if(ans_multiple.size() == 0){
            cout << "No solution" << endl;
        }else{
            double min_E = r;
            for(int i=0;i<ans_multiple.size();i++){
                if(min_E > ans_multiple[i].E){
                    min_E = ans_multiple[i].E;
                }
            }
            cout << "Minimized RMSD: " << min_E << endl;
        }
    }
    return 0;
}